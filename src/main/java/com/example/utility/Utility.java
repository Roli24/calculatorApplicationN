...
import com.example.factory.CalculatorFactory;
...
public class Utility {
        ...
        for (Character input : expression.toCharArray()) {
            ...
            if (token.matches("\\d+(\\.\\d+)?")) {
                ...
                while (!operators.isEmpty() && hasPrecedence(operator, operators.peek())) {
                    ...
                    values.push(performOperation(operators.pop(), operand1, operand2));
                }
                // operators -> +
    ...
    private static double performOperation(Character operator, double operand1, double operand2) {
        ICalculator calculator = CalculatorFactory.createCalculator(operator);
